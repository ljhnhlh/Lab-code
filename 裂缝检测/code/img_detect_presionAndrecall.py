...
def predict_img(model,inputs):
	with torch.no_grad():
		...
		for j in range(inputs.size()[0] - 2):
			if preds[j] == 0 and values[j] == '0':
				std[0][0]+=1
			elif preds[j] == 0 and values[j] == '1':
				std[0][1]+=1
			elif preds[j] == 1 and values[j] == '0':
				std[1][0]+=1
			elif preds[j] == 1 and values[j] == '1':
				std[1][1]+=1
		return std
...
if __name__ == '__main__':
	...
	for i in range(len(filepaths)):
		...
		img_roi.append(img);
	t = classify(img_roi) # 将图片分成imgNum*imgNum份进行识别
